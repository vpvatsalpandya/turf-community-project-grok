...
function vercelHttpsOrigin(raw: string | undefined): string | undefined {
...
// public URL. On the linked Vercel project, fall back to Vercel's production
// domain so email/password is not stuck on grok-sandbox / localhost origins.
const explicitBaseURL =
  env("BETTER_AUTH_URL") ??
  vercelHttpsOrigin(env("VERCEL_PROJECT_PRODUCTION_URL")) ??
  vercelHttpsOrigin(env("VERCEL_URL"));
// Explicit `string[]` (not a readonly tuple) — Better Auth's DynamicBaseURLConfig
// requires a mutable `allowedHosts: string[]`.
const previewAllowedHosts: string[] = [...PREVIEW_ALLOWED_HOSTS];
if (env("VERCEL")) previewAllowedHosts.push("*.vercel.app");
...
const SHIP_ORIGINS: string[] = [
  "https://turf-community.vercel.app",
  "https://turf-community-vatsal-main.vercel.app",
];
...
const trustedOrigins: string[] = explicitBaseURL
  ? [explicitBaseURL, ...SHIP_ORIGINS, ...LOCAL_DEV_ORIGINS]
  : [
